...
| **Task 2 — Chunking, Alt 1: Semantic Chunking** | [reports/Task2_Chunking_Strategy.md](reports/Task2_Chunking_Strategy.md) | [notebooks/02_task2_semantic_chunking.ipynb](notebooks/02_task2_semantic_chunking.ipynb) |
| **Task 2 — Chunking, Alt 2: MarkdownHeaderTextSplitter** | [reports/Task2_Markdown_Chunking.md](reports/Task2_Markdown_Chunking.md) | [notebooks/03_task2_markdown_chunking.ipynb](notebooks/03_task2_markdown_chunking.ipynb) |
...
Baseline = `RecursiveCharacterTextSplitter(500/100)`. Two alternatives implemented, each recovering a chunk the baseline missed on a **different** query type:
...
| **MarkdownHeaderTextSplitter** | dense repetitive **records** | retrieves the exact `#### Error Code E-330` chunk that recursive never surfaces in top-3 → [report](reports/Task2_Markdown_Chunking.md) |

**Key insight:** no single chunking strategy is universally best — it must match the **structure of the data** (semantic for prose/policy, header-based for repetitive records). Each report includes an **end-to-end business-impact** section showing the same prompt + LLM give a *complete/correct* vs *incomplete/wrong* answer depending only on chunking.
...
│   ├── 02_task2_semantic_chunking.ipynb       ← Task 2, Alt 1
│   └── 03_task2_markdown_chunking.ipynb       ← Task 2, Alt 2
└── reports/
    ├── Task1_Prompt_Engineering.md
    ├── Task2_Chunking_Strategy.md             ← Semantic + business impact
    └── Task2_Markdown_Chunking.md             ← MarkdownHeader
```
